  ...
  <div class="row">
    <div class="col-md-8">
      ...
      foreach ($content['data'] as $value) {
        ...
        echo '    <small class="text-muted">'.kdate(from_gmt($value->pub_date)).' | '.$value->author.' | dilihat '.number_format($value->counter, 0).' kali</small>'
            .'    <p>'.$value->description.'</p>';

        if ( ! empty ($value->file1)) {
          $filename = cleanup_basename($value->file1);
          $filename_count = strlen($filename);
          if ($filename_count > 30) {
            $filename = substr($filename, 0, 30).'...';
          }

          echo '<a href="'.base_url($value->file1).'" class="btn btn-info"><span class="glyphicon glyphicon-download-alt"></span> '.$filename.'</a>';
        }
